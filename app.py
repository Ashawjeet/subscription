...
# Route to choose subscription plan and initiate payment
...
def subscribe():
    if request.method == 'POST':
        ...
        return render_template('payment_form.html', plan=plan, amount=amount)
...
def payment_webhook():
    ...
    payload = request.get_json()
    app.logger.info('Webhook Payload: %s', payload)  # Log the payload
    ...
    app.logger.info('Payment Status: %s', payment_status)  # Log the payment status
    app.logger.info('Order ID: %s', order_id)  # Log the order ID
    app.logger.info('Payment ID: %s', payment_id)  # Log the payment ID

    # Fetch the order from the database using the order_id
    razorpay_order = razorpay_client.order.fetch(order_id)

    app.logger.info('Razorpay Order: %s', razorpay_order)  # Log the Razorpay order
    ...
    if payment_status == 'captured':
        ...
        app.logger.info('User ID: %s', user_id)  # Log the user ID
